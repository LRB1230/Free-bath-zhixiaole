...
因软件更新，无法使用老版本进行充值（安装包签名校验），充值请将账号登录到正常新版的手机充值（可以选择找同学登你账号充值，或者，自己有另外一台手机）

# 问题

1.智校乐在26开学这段时间的更新中屏蔽抓包
```

      造成结果：打开抓包智校乐直接断网
            解决方法：使用老版本，并且不更新      
            解决方法2.使用群内的智校乐（去除强制跟新和部分广告）

```
2.智校乐在3.16 加入强制更新
```
      造成结果：进入软件必需更新，不更新无法使用
            解决方法： 充值请将账号登录到同学（正常新版）手机充值
      造成结果：无法使用老版本进行充值
            解决方法：方法1.屏蔽请求跟新的数据包
            解决方法2：使用群内的智校乐（去除强制跟新和部分广告）
```
3.部分寝室使用的计费设备无法破解，randomNumber为8位，校验数据为8位，正常可以破解为4位
```
      因为是个别少数
      暂时没有研究，可能需要从安装包入手
```
...
我们执行的操作是在手机发出数据包的时候拦截下来这个数据包，通过规则（正则表达式）替换，替换水费为0

不能使用在黄鸟普通版，没有规则替换，普通版本没有规则替换，而数据包有随机值校验（防止重发）
# 目前没有针对软件更新，更新对应破解教程
# 如果你已经使用老版本智校乐并且屏蔽更新数据包（暂无教程），成功进入软件，那么跟着下面教程来可以成功复现
# 教程（分步骤）
